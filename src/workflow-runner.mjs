...
export async function runWorkflow({definition,fixture,adapter,failurePolicy={}}){
  ...
  return {ok:blocked.length===0,process_id:definition.id,platform_mode:blocked.length?"degraded":"normal",continue_unblocked:true,results,tickets,escalations,calls:adapter.calls,groups:Object.fromEntries(groups.map((group)=>[group,{completed:results.filter((item)=>item.group===group&&item.ok).length,blocked:results.filter((item)=>item.group===group&&!item.ok).length}]))};
}
